...
lim_max = quantile(areas_poly,probs=c(0.99))
...
```{r }
shape = legend_class_q1[legend_class_q1$OBJECTI == 92081 ,]  #16596 670 51158
shape_buffer = legend_class_buffer_q1[legend_class_buffer_q1$OBJECTI == 92081,]
...
#array_sr = as.array(result_stack)
...
##normalizing data
#data_2d_nonan_norm = func_scale(data_2d_nonan)
##matrix var cov
#R = cov(data_2d_nonan_norm)
#eigen_res = eigen(R)

#P = t(eigen_res$vectors %*% t(data_2d_nonan_norm))
...
#for(l in 2:6){
#  km <- kmeans(P,l)
#  a[l-1] = round(fpc::calinhara(P,clustering = km$cluster, cn = l),digits=2)
#  }
#centers = order(a,decreasing = TRUE)[1] + 1
#cluster_data_2d = kmeans(P,centers, iter.max = 10)

#vector_cluster = c(array(NA, dim = dim(array_sr)[1:2]))

#vector_cluster[!index_nonan]<-cluster_data_2d$cluster

#cluster_matrix = array(vector_cluster, dim = dim(array_sr)[1:2])
...
#st_write(as.data.frame(result_clip),"output_clip_P2_ndvi.csv",layer_options = "GEOMETRY=AS_XY")
```
...
file_index = 'C:\\IPSTERS\\sraster\\ins4\\sampling_0\\output_water_norule.csv'
...
```{r }
#query 2  eval= FALSE 
#legend_class_buffer_q1$OBJECTI %in% 
