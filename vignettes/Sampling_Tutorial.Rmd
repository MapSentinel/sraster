...
```{r }
shape = legend_class_q1[legend_class_q1$OBJECTID == 5949,]  
shape_buffer = legend_class_buffer_q1[legend_class_q1$OBJECTID == 5949,]
...
```{r }
result_clip = clip_sraster(result, mask = cluster_matrix, type ="rule_ndvi_urban")
plot(funct_plot((result_clip$array)[,,1]))
```
...
result_list = lapply(list_shapes, workflow, paths_images, type ="rule_ndvi_urban")
...
st_write(result_df2, paste0("output_",class_analysis,"_ndvirule1.csv"), layer_options = "GEOMETRY=AS_XY")
```
