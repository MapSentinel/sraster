...
#buffer
legend_water_buffer = st_buffer(legend_water,-20)
table(legend$Legend)
...
```{r }
shape = legend_water[legend_water$OBJECTID == 92266 ,]
shape_buffer = legend_water_buffer[legend_water_buffer$OBJECTID == 92266 ,]
...
query_water_buffer = legend_water_buffer[index,]
```
...
list_shapes_buffer = split(query_water_buffer,query_water_buffer$OBJECTID)

workflow <- function(shape,paths_images){
          result = as_sraster(shape,paths_images)
          cluster_matrix = kmeans_sraster(result)
          result_clip = clip_sraster(result, mask = cluster_matrix, type ="rule_ndvi_water")  # Majority rule
...
st_crs(result_df)<-st_crs(legend)
st_write(result_df, "output_water_clean_2.csv", layer_options = "GEOMETRY=AS_XY")
```
